...
waypoint_1 = LocationGlobalRelative(38.3707123, 27.2009484, 7)
"""
waypoints = {
...
#-------Mission--------------------------
Land_Target = False # Araç Land mi atsın 
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        if Use_Circle_Check:
            ...
            if target:
                ...
                if (v_alt - 0.2) <= 4 and abs(cY - 240) <= land_sensivity and abs(cX - 320) <= land_sensivity:
                    if Land_Target:
            ...
            # Eğer daireyi görmüyorsa
                ...
                #---------------------------------------------------------------------------------------
                if v_alt <= 4 and not ptin_contour:
                    Velocity_z = 0
                    print("Arac 4m'ye indi zorunlu LAND atiyor...")
                    vehicle.mode = VehicleMode("LAND")
...
if user_approval == "arm":
    ...
    if TargetCompleted:
        ...
        time.sleep(5)
        vehicle.simple_goto(waypoint_base,groundspeed=3)
